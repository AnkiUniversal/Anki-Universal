...
namespace AnkiU.AnkiCore
{
    ...
    public class Card
    {
        ...
        private Dictionary<string, string> questionAndAnswer;
        private Note note;
        ...
        public void SaveChangesToDatabase(bool changeModUsn = true)
        {
            if (changeModUsn)
            {
                timeModified = DateTimeOffset.Now.ToUnixTimeSeconds();
                usn = collection.Usn;
            }
            // bug check
            if ((queue == 2 && oDue != 0) && !collection.Deck.IsDyn(deckId))
            {
                // TODO: runHook("odueInvalid");
            }
            Debug.Assert(due < 4294967296);
            ...
            collection.Log(args: this);
        }

        public void SaveSchedToDatabase()
        {
            timeModified = DateTimeOffset.Now.ToUnixTimeSeconds();
            usn = collection.Usn;
            // bug check
            ...
            collection.Database.Execute(
            "update cards set mod =?, usn =?, type =?, queue =?, due =?, ivl =?, factor =?, reps =?, " +
        ...
        public string GetQuestionWithCss(bool reload = false, bool browser = false)
        {
            return Css() + GetQuestionAndAnswer(reload, browser)["q"];
        }

        public string GetAnswerWithCss()
        {
            return Css() + GetQuestionAndAnswer()["a"];
        }

        public Dictionary<string, string> GetQuestionAndAnswer(bool reload = false, bool browser = false)
        {
            if (questionAndAnswer == null || reload)
            {
                ...
                if (browser)
                {
                    string bqfmt = t.GetNamedString("bqfmt");
                    string bafmt = t.GetNamedString("bafmt");
                    questionAndAnswer = collection.RenderQA(data, bqfmt, bafmt);
                }
                else
                {
                    questionAndAnswer = collection.RenderQA(data);
                }
            }
        ...
        {
            return collection.Models.Get(LoadNote().ModelId);
        }

        public Note LoadNote(bool reload = false)
        {
            if (note == null || reload)
            {
        ...
        public bool IsEmpty()
        {
            List<int> ords = collection.Models.AvailableOrds(GetModel(), Utils.JoinFields(LoadNote().Fields));
            return !ords.Contains(ord);
        }

        //***********************************************************
        //The methods below are not in the python version
        //***********************************************************
        public string QSimple()
        {
            return GetQuestionAndAnswer(false)["q"];
        }
        ...
        public override string ToString()
        {
            ...
            foreach (FieldInfo f  in fields)
            {
                ...
                members.Add(String.Format(Media.locale, "'{0}': {1}", f.Name, this.GetType().Name));
            }
