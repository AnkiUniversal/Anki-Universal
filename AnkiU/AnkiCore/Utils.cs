...
namespace AnkiU.AnkiCore
{
    public static class Utils
    {
        ...
        /// </summary>
        /// <param name="time_s">The time to format, in seconds</param>
        /// <returns>The time quantity string. Something like "3 s" or "1.7 yr"</returns>
        public static string TimeQuantity(int time_s)
        {
            TimeSpan time = TimeSpan.FromSeconds(Math.Abs(time_s));
            
            if (time.TotalSeconds < 60)
            {
                return Math.Round(time.TotalSeconds).ToString("0.#", Media.locale) + " s";
            }
            else if (time.TotalMinutes < 60)
            {
                return time.TotalMinutes.ToString("0.#", Media.locale) + " min";
            }
            else if (time.TotalHours < 24)
            {
                return time.TotalHours.ToString("0.#", Media.locale) + " h";
            }
            else if (time.TotalDays < 30)
            {
                return time.TotalDays.ToString("0.#", Media.locale) + " d";
            }
            else if (time.TotalDays < 365)
            {
                return (time.TotalDays/30).ToString("0.#", Media.locale) + " mon";
            }
            else
            {
                return (time.TotalDays/365).ToString("0.#", Media.locale) + " y";
            }
        }
